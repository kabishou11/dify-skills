...
| `Messages.create() got an unexpected keyword argument 'temperature'` (minimax) | plugin 0.0.27 switched to Anthropic-compatible channel; SDK rejects `temperature/top_p` kwargs | downgrade to 0.0.26 (pack from daemon cwd old dir, uninstall 0.0.27 `preserve_credentials:true`, install 0.0.26); regression = only nodes with sampling params fail, nodes without them work fine (source: `_functional.py:97-101` sampling tuple) |
